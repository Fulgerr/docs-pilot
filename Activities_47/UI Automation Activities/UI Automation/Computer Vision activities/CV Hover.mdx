...
Hovers over a specified UI element which is targeted by using the UiPath Computer
...
* DelayAfter - Delay time (in milliseconds) after executing the activity. By default, this field is empty.
* DelayBefore - Delay time (in milliseconds) before the activity begins performing any operations. By default, this field is empty.
...
* TimeoutMS - Specifies the amount of time (in milliseconds) to wait for the target to be found before an error is thrown. By default, this field is empty.
...
* CursorMotionType - Specifies the type of motion performed by the mouse cursor. There are two available options:Instant - The cursor jumps to the destination Smooth - The cursor moves gradually towards the destination. This option is suitable for elements that respond to interaction only when the mouse smoothly enters their area. Has no effect if SendWindowMessages or SimulateClick are enabled.
