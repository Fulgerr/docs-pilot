...
This activity is available starting with v2017.1.

`UiPath.Core.Activities.SelectMultipleItems`

Selects multiple items from a combo box or list box. After you indicate the UI element on screen, add the labels you want to select in the MultipleItems property. At runtime, the specified items are selected.
...
* MultipleItems - Specifies the items to be selected in the list box.
* AddToSelection - Specifies if the items will be added to the existing selection or if the list box selection is first cleared.
...
# Example of using the Select Multiple Items activity

Here you can see how the Select Multiple Items activity is used in an example that incorporates multiple activities.
