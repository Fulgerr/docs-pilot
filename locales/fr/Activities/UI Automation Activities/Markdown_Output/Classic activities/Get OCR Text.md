...
UiPath.Core.Activities.GetOCRTextWithBodyFactory

Extracts a string and its information from an indicated UI element or image using the OCR screen scraping method. This activity can also be automatically generated when performing screen scraping, along with a container. By default, the Google OCR engine is used.

# Properties

Output

* Text - The string extracted from the indicated UI element.
* WordsInfo - The screen coordinates of each word found in the indicated UI element. This field supports only TextInfo variables.
...
# Example of using the Get OCR Text activity

Here you can see how the Get OCR Text activity is used in an example that incorporates multiple activities.
